...
"""
╔══════════════════════════════════════════════════════════════════╗
║  BOT CARO RAPFI - FULL NAME + AVATAR v6.0                     ║
║  Engine: Rapfi 2025-06-15 (Rank #1 Gomocup 2026)                    ║
...
# --- Rapfi 2025 (Rank #1 Gomocup 2026) ---
AG_BINARY = "pbrain-rapfi-linux-clang-avx2"
...
def auto_download_engine(binary_name: str, download_url: str, fmt: str = "bz2") -> Optional[str]:
    ...
    try:
        ...
        if binary_path.exists():
            ...
            return str(binary_path)
        # Fallback: tìm binary Linux trong ENGINE_DIR
        for f in ENGINE_DIR.glob(binary_name + "*"):
            name = f.name.lower()
            if f.is_file() and "windows" not in name and "macos" not in name:
                f.chmod(0o755)
                return str(f)
...
def detect_rapfi_binary() -> Optional[str]:
    """Tìm Rapfi Linux binary (ưu tiên AVX2)."""
    if not ENGINE_DIR.exists(): return None
    for pat in ["pbrain-rapfi-linux-clang-avx2", "pbrain-rapfi-linux-clang-avx512",
                 "pbrain-rapfi-linux-clang-avxvnni", "pbrain-rapfi-linux-clang-sse"]:
...
# ======================== ENGINE WRAPPER ========================
class RapfiEngineWrapper:
    """Wrapper cho Rapfi - engine #1 Gomocup 2026."""
    def __init__(self, timeout_turn=5000, board_size=15, rule=1):
        ...
        self.binary = detect_rapfi_binary()
    ...
    def start_game(self, my_symbol=1) -> bool:
        ...
        try:
            ...
            # Rapfi: START protocol (hỗ trợ mọi kích thước bàn)
            ...
            time.sleep(0.3)
            self._initialized = True
            log.info(f"[RAPFI] Started (board={self.board_size})")
            return True
        except Exception as e:
            log.error(f"[RAPFI] Start error: {e}")
            self._initialized = False
            return False
    ...
    def restart_game(self) -> bool:
        with self.lock:
            ...
            for _ in range(5):
                line = self._read_line(timeout=2.0)
                if line.upper() == "OK":
                    log.info("[RAPFI] RESTART successful")
                    return True
            log.warning("[RAPFI] RESTART did not return OK, assuming reset")
            return True
    ...
    def get_move(self, board_history: list, my_side: int) -> Optional[Tuple[int, int]]:
        with self.lock:
            try:
                ...
                log.warning(f"[RAPFI] get_move error: {e}")
                self._synced = False
                return None
...
class CaroBot:
    ...
    def init_ag(self):
        ...
        binary = detect_rapfi_binary()
        if not binary:
            binary = auto_download_engine(AG_BINARY, AG_DOWNLOAD_URL, fmt="7z")
        
        if not binary:
            log.warning("[RAPFI] No binary found!")
            self.ag_available = False
            return False
        
        try:
            self.ag = RapfiEngineWrapper(
                timeout_turn=AG_TIMEOUT, board_size=self.board.width, rule=AG_RULE
            )
            self.ag.binary = binary
            ok = self.ag.start_game(my_symbol=self.my_symbol)
            if ok:
                self.ag_available = True
                log.info(f"[RAPFI] v{AG_VERSION} OK! Board={self.board.width}")
            else:
                self.ag_available = False
                log.warning("[RAPFI] Start failed!")
            return self.ag_available
        except Exception as e:
            log.error(f"[RAPFI] Init error: {e}")
            self.ag_available = False
            return False
            ...
            if self.ag_available:
                try:
                    ...
                    if (move and 0 <= move[0] < self.board.width and 0 <= move[1] < self.board.height
                        ...
                        self.ag_errors += 1
                        log.warning(f"[RAPFI] Nước không hợp lệ: {move}, fallback gần nước cuối + hard reset")
                        if history:
                    ...
                    self.ag_errors += 1; log.warning(f"[RAPFI] Error: {e}")
