...
"""
╔══════════════════════════════════════════════════════════════════╗
║  BOT CARO STARDUST - FULL NAME + AVATAR v1.0                   ║
║  Engine: Stardust 2019 (Standard/Caro) via Piskvork            ║
║  TEST: Đã test RECTSTART 15,19 -> OK (hỗ trợ bàn 15x19)        ║
║  FIX: Giữ nguyên logic bot2.py (AlphaGomokuEngine) chuẩn       ║
...
ENGINE_DIR = _BASE_DIR / "stardust-engine"
AG_BINARY = "pbrain-stardust-tm-std1-adppos-adpmink1k-actp0.95-0.1-ch3-cost-mem-nl.exe"
AG_VERSION = "2019"
AG_DOWNLOAD_URL = "http://download.gomocup.com/ai/STARDUST.zip"
AG_RULE = 8  # Caro rule - đã test RECTSTART 15,19 -> OK
...
def auto_download_alphagomoku() -> Optional[str]:
    ...
    log.info(f"[AG] Downloading Stardust {AG_VERSION}...")
    ENGINE_DIR.mkdir(parents=True, exist_ok=True)
    try:
        import zipfile
        archive = Path("/tmp/stardust.zip")
...
def detect_ag_binary() -> Optional[str]:
    ...
    # Stardust 2019: Windows exe chạy qua wine (kèm models/474/)
...
PASSWD = _clean_env(os.environ.get("CARO_PASSWD1") or os.environ.get("CARO_PASSWD"), CARO_PASSWD_DIRECT)
# Nếu muốn chỉ dùng hardcode:
# USER = "stardust1"
# PASSWD = "nhat123456"
if not USER or not PASSWD:
    print("[BOT] Thiếu CARO_USER / CARO_PASSWD - kiểm tra hardcode trong STARDUST.py")
    sys.exit(1)
VERSION = "5.0.2"
GAME_ID = "caro"
...
class CaroBot:
    ...
    def init_ag(self):
        ...
        try:
            ...
            if ok:
                ...
                log.info(f"[AG] Stardust v{AG_VERSION} OK! Rule={AG_RULE}")
            else:
                self.ag_available = False
    ...
    async def run(self):
        ...
        log.info(f"{'='*50}")
        log.info("BOT CARO STARDUST - FULL_NAME + AVATAR v1.0")
        log.info(f"{'='*50}")
...
def main():
    ...
    if bin_path: print(f"[SETUP] Stardust ready: {os.path.basename(bin_path)}")
    else: print("[SETUP] No Stardust - bot plays center only")
