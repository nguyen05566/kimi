...
"""
╔══════════════════════════════════════════════════════════════════╗
║  BOT CARO ANGEL26 - NATIVE 15x19 + TACTICAL GUARD v1.0       ║
║  Engine: ANGEL26 policy network via Wine                        ║
...
ANGEL_DIR = _BASE_DIR / "angel26-engine"
ANGEL_BINARY = "pbrain-angel2026.exe"
ANGEL_VERSION = "ANGEL26"
ANGEL_DOWNLOAD_URL = "https://download.gomocup.com/ai/ANGEL26.zip"
ANGEL_RULE = 8  # Caro flag; Angel parses it, tactical guard enforces critical threats.
ANGEL_TIMEOUT = 5000
WINE_PREFIX = _BASE_DIR / ".wine-angel26"
...
def auto_install_wine() -> Optional[str]:
    ...
    if not shutil.which("apt-get"):
        print("[ANGEL] Wine not found and apt-get is unavailable")
        return None
    sudo = ["sudo"] if shutil.which("sudo") else []
    print("[ANGEL] Installing wine64 (one-time runner setup)...")
    try:
        subprocess.run(
        ...
        print(f"[ANGEL] Wine installation failed: {e}")
        return None
    return detect_wine_binary()


def auto_download_angel() -> Optional[str]:
    binary_path = ANGEL_DIR / ANGEL_BINARY
    model_path = ANGEL_DIR / "model.bin"
    if binary_path.exists() and model_path.exists():
        return str(binary_path)

    print(f"[ANGEL] Downloading {ANGEL_VERSION}...")
    ANGEL_DIR.mkdir(parents=True, exist_ok=True)
    archive = Path("/tmp/ANGEL26.zip")
    try:
        import zipfile
        request = urllib.request.Request(
            ANGEL_DOWNLOAD_URL,
            headers={"User-Agent": "Mozilla/5.0 (GitHub Actions ANGEL26 bot)"})
        with urllib.request.urlopen(request, timeout=120) as response, open(archive, "wb") as out:
            shutil.copyfileobj(response, out)
        with zipfile.ZipFile(archive, "r") as zf:
            zf.extractall(ANGEL_DIR)
        archive.unlink(missing_ok=True)
        if not binary_path.exists() or not model_path.exists():
            raise FileNotFoundError("ANGEL26 binary/model missing after extraction")
        return str(binary_path)
    except Exception as e:
        print(f"[ANGEL] Download failed: {e}")
        return None


def auto_setup_angel() -> Optional[str]:
    binary = auto_download_angel()
    if not binary:
        return None
...
class AngelGomokuEngine:
    """ANGEL26 policy-network pbrain with native RECTSTART 15x19 support."""

    def __init__(self, timeout_turn=5000, board_size=15, rule=8, width=None, height=None):
        self.binary = str(ANGEL_DIR / ANGEL_BINARY) if (ANGEL_DIR / ANGEL_BINARY).exists() else None
    ...
    def _read_move(self, timeout: float) -> Optional[Tuple[int, int]]:
        ...
        while time.monotonic() < deadline:
            ...
            if upper.startswith("ERROR"):
                log.warning(f"[ANGEL] Engine error: {line}")
                return None
        return None
        ...
        with self.lock:
            ...
            try:
                ...
                self.proc = subprocess.Popen(
                    ...
                    stderr=subprocess.DEVNULL, cwd=str(ANGEL_DIR), env=env)
                self._buffer = b""
                self._send(f"RECTSTART {self.width},{self.height}")
                status = self._read_status(timeout=60.0)
                if not status or not status.upper().startswith("OK"):
                    log.warning(f"[ANGEL] RECTSTART failed: {status!r}")
                    self.stop()
                    return False
                self._send_info()
                ...
                log.info(
                    f"[ANGEL] Started native {self.width}x{self.height}, rule={self.rule}")
                return True
            except Exception as e:
                log.error(f"[ANGEL] Start error: {e}")
                self.stop()
                return False
            ...
            try:
                ...
                if use_turn:
                    ...
                    if last_symbol == self.my_side:
                        ...
                        log.info(f"[ANGEL] TURN {int(last_x)},{int(last_y)}")
                if not use_turn:
                    self._send("BOARD")
                    ...
                    self._send("DONE")
                    log.info(f"[ANGEL] BOARD resync; stones={len(board_history)}")

                move = self._read_move(timeout=max(10.0, self.timeout_turn / 1000.0 + 5.0))
                if move is None:
                    log.warning("[ANGEL] Timed out or returned no coordinate")
                    self.invalidate_sync()
                    return None
                x, y = move
                if not (0 <= x < self.width and 0 <= y < self.height):
                    log.warning(
                        f"[ANGEL] Invalid move outside {self.width}x{self.height}: {move}")
                    self.invalidate_sync()
                    return None
                self._synced = True
                ...
                log.warning(f"[ANGEL] get_move error: {e}")
                self.invalidate_sync()
                return None
...
class Board:
    ...
    def forced_tactical_move(self, my_symbol: int,
        ...
        """Handle wins, direct blocks and open-four prevention before ANGEL."""
...
class CaroBot:
    ...
    def init_ag(self):
        ...
        binary = ANGEL_DIR / ANGEL_BINARY
        runtime_ok = os.name == "nt" or detect_wine_binary() is not None
        if not binary.exists() or not runtime_ok:
            log.warning("[ANGEL] No binary or Wine runtime!")
            self.ag_available = False
            return False
        try:
            self.ag = AngelGomokuEngine(timeout_turn=ANGEL_TIMEOUT, board_size=max(self.board.width, self.board.height), rule=ANGEL_RULE, width=self.board.width, height=self.board.height)
            ok = self.ag.start_game(my_symbol=self.my_symbol, width=self.board.width, height=self.board.height)
            if ok:
                self.ag_available = True
                log.info(f"[ANGEL] OK! Rule={ANGEL_RULE} native {self.board.width}x{self.board.height}")
            else: self.ag_available = False; log.warning("[ANGEL] Start failed!")
            return self.ag_available
        except Exception as e: log.error(f"[ANGEL] Init error: {e}"); self.ag_available = False; return False
        ...
        try:
            ...
            if tactical_move is not None:
                ...
                log.warning(
                    ...
                    "ưu tiên hơn nước ANGEL")
                # ANGEL did not produce this move, so its internal tree is stale.
                if self.ag:
                ...
                try:
                    ...
                    if (move and 0 <= move[0] < self.board.width and 0 <= move[1] < self.board.height
                        ...
                        self.ag_errors += 1
                        log.warning(f"[ANGEL] Nước không hợp lệ: {move}; BOARD resync và thử lại một lần")
                        self.ag.invalidate_sync()
                        ...
                        if (retry_move and 0 <= retry_move[0] < self.board.width
                            ...
                            x, y = retry_move
                            self.ag_moves += 1
                            log.info(f"[ANGEL] Resync retry OK: {retry_move}")
                        else:
                            log.warning(f"[ANGEL] Resync retry failed: {retry_move}; dùng fallback")
                            if history:
                    ...
                    self.ag_errors += 1; log.warning(f"[ANGEL] Error: {e}")
            ...
            log.info(f"MOVE ({x},{y}) took {elapsed:.2f}s [ANGEL]")
    ...
    async def run(self):
        ...
        log.info(f"{'='*50}")
        log.info("BOT CARO ANGEL26 - NATIVE 15x19 + TACTICAL GUARD v1.0")
        log.info(f"{'='*50}")
...
def main():
    bin_path = auto_setup_angel()
    if bin_path: print(f"[SETUP] ANGEL26 ready: {os.path.basename(bin_path)}")
    else: print("[SETUP] No ANGEL26/Wine - bot uses fallback moves")
